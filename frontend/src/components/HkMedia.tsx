...
export function HkTaskMediaPanel({ row }: { row: Row; reload: () => void }) {
  ...
  const loadMedia = useCallback(() => {
    call<string[]>("hotelpms.api.hk_task_media", { task: row.name })
      .then(setUrls)
  ...
  async function del(url: string) {
    ...
    try {
      await call("hotelpms.api.hk_delete_media", { task: row.name, file_url: url })
      loadMedia()
    } catch (e) {
...
export function RoomMediaButton({ room }: { room: string }) {
  ...
  async function show(e: React.MouseEvent) {
    ...
    if (urls === null) {
      try {
        setUrls(await call<string[]>("hotelpms.api.hk_room_media", { room }))
      } catch {
        setUrls([])
      }
    }
