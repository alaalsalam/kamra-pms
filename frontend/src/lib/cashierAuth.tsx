...
export function CashierAuthProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const s = await call<PinStatus>("hotelpms.api.cashier_pin_status", {
  ...
  const onSubmit = useCallback(
    async (pin: string) => {
      ...
      try {
        if (padMode === "enroll") {
          await call("hotelpms.api.set_cashier_pin", { pin })
          await call("hotelpms.api.verify_cashier_pin", {
            property: getCurrentProperty(),
            pin,
          })
        } else {
  ...
  const ensureUnlocked = useCallback(() => {
    return new Promise<void>((resolve, reject) => {
      void (async () => {
        const s =
          status ??
          (await call<PinStatus>("hotelpms.api.cashier_pin_status", {
            property: getCurrentProperty(),
          }).catch(() => null))
        if (s) setStatus(s)
