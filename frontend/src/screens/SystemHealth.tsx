...
type HealthPayload = {
  ...
  installed: { hotelpms: string; frappe: string | null; site: string }
  latest: {
    ok: boolean
    tag: string | null
    name: string | null
...
export default function SystemHealth() {
  ...
  const load = useCallback(async (refresh = false) => {
    ...
    setError(null)
    try {
      const out = await call<HealthPayload>("hotelpms.health.system_health", {
        refresh: refresh ? 1 : 0,
      })
      setData(out)
    } catch (e) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-zinc-500">
            {t("Check the status of this HotelPMS site and whether a newer open-source release is available.")}
          </p>
        </div>
        <Button
          variant="outline"
            ...
            <CardContent className="grid gap-3 sm:grid-cols-3">
              <div className="rounded-lg border border-zinc-100 bg-zinc-50 px-3 py-2">
                ...
                <p className="mt-0.5 font-mono text-sm font-semibold">
                  HotelPMS {data.installed.hotelpms}
                </p>
