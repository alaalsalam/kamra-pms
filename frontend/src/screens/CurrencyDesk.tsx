...
export default function CurrencyDesk() {
  ...
  const load = useCallback(async () => {
    try {
      const r = await call<{ rates: typeof rates }>(
        "hotelpms.ledger.list_exchange_rates",
        { property: getCurrentProperty() },
      )
      setRates(r.rates)
      const h = await call<{ transactions: Record<string, unknown>[] }>(
        "hotelpms.ledger.exchange_history",
        { property: getCurrentProperty() },
      )
  ...
  const doCalc = async () => {
    try {
      const out = await call<Record<string, number | string>>(
        "hotelpms.ledger.currency_calculator",
        {
          property: getCurrentProperty(),
          currency,
  ...
  const post = async () => {
    try {
      await withPin(async (pin) => {
        await call("hotelpms.ledger.post_exchange", {
          property: getCurrentProperty(),
          currency,
          fx_amount: Number(fxAmount),
          direction,
  ...
  const saveRate = async () => {
    try {
      await call("hotelpms.ledger.upsert_exchange_rate", {
        property: getCurrentProperty(),
