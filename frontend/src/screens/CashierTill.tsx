...
export default function CashierTill() {
  ...
  const load = useCallback(async () => {
    try {
      const s = await call<Session>("hotelpms.cashier.current_session", {
  ...
  const open = () =>
    act(async () => {
      ...
      const s = await call<Session>("hotelpms.cashier.open_session", {
        property: getCurrentProperty(),
  ...
  const close = () =>
    act(async () => {
      ...
      await withPin(async (pin) => {
        await call("hotelpms.cashier.close_session", {
  ...
  const paidOut = () =>
    act(async () => {
      await withPin(async (pin) => {
        await call("hotelpms.cashier.post_paid_out", {
          property: getCurrentProperty(),
  ...
  const drop = () =>
    act(async () => {
      await withPin(async (pin) => {
        await call("hotelpms.cashier.cash_drop", {
          property: getCurrentProperty(),
