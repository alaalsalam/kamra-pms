...
class HousekeepingTask(Document):
	...
	def on_update(self):
		...
		if self.status == "Done":
			...
			from hotelpms.housekeeping import notify_room_ready
			notify_room_ready(self)
			try:
				from hotelpms.turnover import on_checkout_clean_done
			...
			profile = None
			try:
				from hotelpms.turnover import get_turnover_profile, mark_room_ready
