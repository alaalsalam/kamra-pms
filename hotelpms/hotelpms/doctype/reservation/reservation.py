...
class Reservation(Document):
	def after_insert(self):
		...
		if self.status == "Confirmed":
			from hotelpms import whatsapp
			if whatsapp.has_connection(self.property):
				frappe.enqueue(
					"hotelpms.whatsapp.notify_booking_confirmed",
	...
	def validate_type_capacity(self):
		...
		from hotelpms.reservation_state import holds_inventory
	...
	def validate_status_transition(self):
		...
		if not old:
			return
		from hotelpms.reservation_state import assert_transition
	...
	def validate_cancellation_path(self):
		...
		if self.is_new() or frappe.flags.hotelpms_cancelling:
			return
		old = self.get_doc_before_save()
	...
	def validate_room_belongs_to_type(self):
		...
		if room_type != self.room_type:
			...
			from hotelpms.api import _available_rooms_raw
			free = [
	...
	def validate_no_overlap(self):
		...
		This check is the seed of HotelPMS's no-overbooking guarantee: it runs
	...
	def handle_check_out(self):
		...
		from hotelpms.folio import post_remaining_nights
