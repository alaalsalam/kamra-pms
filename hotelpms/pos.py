...
POS_ROLES = ("Front Desk", "Finance", "Restaurant POS", "HotelPMS Agent")
KITCHEN_ROLES = ("Kitchen", "Restaurant POS", "HotelPMS Agent")
...
@frappe.whitelist()
@require_roles(*POS_ROLES, *KITCHEN_ROLES)
...
def in_house_rooms(property: str):
	"""Minimal room-service picker: occupied room number + reservation only.

	Unlike the housekeeping queue this does not expose room-cleaning tasks or
	guest operations to restaurant staff.
	"""
	stays = frappe.get_all(
		"Reservation",
		filters={"property": property, "status": "Checked In", "room": ["is", "set"]},
		fields=["name as reservation", "room", "guest_name"],
		order_by="room asc",
	)
	room_numbers = dict(frappe.get_all(
		"Room",
		filters={"name": ["in", [s.room for s in stays]]},
		fields=["name", "room_number"],
		as_list=True,
	)) if stays else {}
	return [{**s, "room_number": room_numbers.get(s.room) or s.room.split("-")[-1]}
		for s in stays]
...
def _cleaning_key(outlet: str, table_no: str) -> str:
	return f"hotelpms_pos_cleaning|{outlet}|{table_no}"
...
def fire_kot(order: str, course: str | None = None):
	...
	# reports itself as stale. See hotelpms/inventory.py.
	from hotelpms.inventory import consume_for_lines
...
def pay_order(order: str, mode: str):
	...
	if mode not in ("Cash", "Card", "Mada", "Digital Wallet"):
		frappe.throw(_("Pick a payment mode: Cash, Card, Mada or Digital Wallet."))
	doc = frappe.get_doc("POS Order", order)
