...
def _room_gst(reservation) -> float:
	...
	from hotelpms.pricing import room_gst_rate
...
def _nightly_room_rate(reservation, date) -> float:
	...
	from hotelpms.pricing import (occupancy_rate, rates_include_tax,
...
def _nightly_gst(reservation, date) -> float:
	from hotelpms.pricing import (occupancy_rate, room_gst_rate, season_adjust)
...
def cancel_invoice(folio_name: str, reason: str) -> dict:
	...
	frappe.flags.hotelpms_invoice_cancel = True
	try:
		folio.save(ignore_permissions=True)
	finally:
		frappe.flags.hotelpms_invoice_cancel = False
