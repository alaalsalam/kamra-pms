...
whitelisted HotelPMS endpoint therefore declares who may call it."""
...
def require_roles(*roles):
	...
	def deco(fn):
		...
		def guarded(*args, **kwargs):
			...
			return fn(*args, **kwargs)
		# introspectable RBAC: HotelPMS Agent filters its tool list by this
		guarded._hotelpms_roles = allowed
		return guarded
	return deco
...
def require_cashier_pin(property: str, pin=None):
	...
	Skipped for agents (HotelPMS Agent role, the copilot's in-process tool calls,
	...
	if not property or not frappe.db.get_value(
		...
		return
	if getattr(frappe.flags, "hotelpms_agent_call", False) or \
	   getattr(frappe.flags, "hotelpms_gate_bypass", False):
		return
	if "HotelPMS Agent" in frappe.get_roles():
		return
