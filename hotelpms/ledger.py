...
@frappe.whitelist()
@require_roles("Finance", "Front Desk", "Hotel Admin", "HotelPMS Agent")
...
@frappe.whitelist()
@require_roles("Finance", "Hotel Admin", "HotelPMS Agent")
...
@frappe.whitelist()
@require_roles("Finance", "Hotel Admin", "Front Desk", "HotelPMS Agent")
...
def force_advance_bill(reservation: str, nights: str = "entire",
	...
	"""
	from hotelpms.folio import (open_folio, post_room_night, _nightly_room_rate,
	                         _recalculate)
	from hotelpms.authz import require_cashier_pin
...
def generate_proforma(folio: str, notes: str | None = None):
	from hotelpms.api import folio_invoice
...
@frappe.whitelist()
@require_roles("Front Desk", "Finance", "Hotel Admin", "HotelPMS Agent")
...
def quick_checkout(reservation: str, pin: str | None = None):
	...
	from hotelpms.api import close_folio, check_out
	res = frappe.get_doc("Reservation", reservation)
...
def post_exchange(property: str, currency: str, fx_amount: float,
	...
	from hotelpms.authz import require_cashier_pin
	from hotelpms.cashier import require_open_session, record_cashier_txn
