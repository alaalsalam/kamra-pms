...
from hotelpms.mcp_tools import TOOL_COUNT
...
def connect_info(property: str) -> dict:
	...
	return {
		...
		"claude_code": f"claude mcp add --transport http hotelpms {mcp_url}",
...
def _error_page(message: str) -> str:
	return _html_shell(
		...
		"<p><a href='/hotelpms/assistant'>Back to HotelPMS</a></p>",
	)
...
def _consent_page(params: dict[str, str], user: str, properties: list[dict]) -> str:
	...
	return _html_shell(
		...
		as the desk. Prices, taxes and availability stay in HotelPMS. Every
...
<title>{frappe.utils.escape_html(title)} · HotelPMS</title>
<style>
