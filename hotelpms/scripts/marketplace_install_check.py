...
Mirrors the checks Frappe Cloud and CI run before a site can serve HotelPMS:
...
def check_pyproject() -> None:
	...
	if project.get("name") != "hotelpms":
		fail("pyproject name is not hotelpms")
...
def check_hooks() -> None:
	...
	if ns.get("app_name") != "hotelpms":
		fail("hooks.app_name is not hotelpms")
	...
	if not screen or screen[0].get("route") != "/hotelpms":
		fail("add_to_apps_screen missing /hotelpms route")
	rules = str(ns.get("website_route_rules"))
	if "/hotelpms/<path:app_path>" not in rules:
		fail("/hotelpms SPA route rule missing")
	if ns.get("app_license") != "agpl-3.0":
		fail("app_license is not agpl-3.0")
	ok("hooks.py: payments, /hotelpms launcher, SPA route, AGPL")


def check_shipped_spa() -> None:
	index = APP / "public" / "frontend" / "index.html"
	boot = APP / "www" / "hotelpms.py"
	if not index.is_file():
		fail("prebuilt SPA missing at hotelpms/public/frontend/index.html")
	if not boot.is_file():
		fail("SPA boot page missing at hotelpms/www/hotelpms.py")
	html = index.read_text()
	if "/assets/hotelpms/frontend/assets/" not in html:
		fail("SPA index.html does not reference /assets/hotelpms/frontend/assets/")
...
def main() -> None:
	print("HotelPMS marketplace install simulation (offline)\n")
	check_pyproject()
	check_hooks()
	check_shipped_spa()
	...
	print("  bench get-app payments && bench get-app hotelpms && install-app")
