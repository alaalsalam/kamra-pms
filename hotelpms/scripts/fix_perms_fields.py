...
2. Custom DocPerm rows REPLACE a doctype's built-in permissions in
   ...
   System Manager full custom perms on every HotelPMS doctype, and give
   Front Desk / Finance their missing folio-era grants.
3. The same trap, one layer deeper: seed_rbac_v2.ensure_agent_user()
   writes Custom DocPerms for HotelPMS Agent, and the moment ONE lands the
   ...
   SPA mostly keeps working because HotelPMS's whitelisted endpoints go
...
Run via bench console:
    from hotelpms.scripts.fix_perms_fields import execute; execute()
"""
