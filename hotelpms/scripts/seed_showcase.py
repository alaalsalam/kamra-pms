...
Run with:
    bench --site <site> execute hotelpms.scripts.seed_showcase.execute
...
def execute():
	...
	from hotelpms import inventory
	...
	if (inhouse and frappe.db.exists("Laundry Rate", {"property": PROPERTY})
		...
		from hotelpms.laundry import collect_laundry, laundry_status
...
def seed_sample_content():
	...
	PROFILE = {
		"website": "https://demo.hotelpms.yemenfrappe.com",
		"driving_directions": (
		...
		"meta_title": "فندق نُزُل الرياض | Nuzul Riyadh Hotel, Bengaluru - boutique stays near Lalbagh",
		"meta_description": (
			"38 rooms of quiet luxury by Lalbagh Botanical Garden. Direct "
			"rates, pay at hotel, instant confirmation."),
		"page_slug": "hotelpms-demo-palace",
		"overbooking_pct": 10,
	}
	...
	if not frappe.db.exists("WhatsApp Message", {"property": PROPERTY}):
		...
		thread = [
			("Outbound", "Template", "hotelpms_booking_confirmation", "Sent",
			 "Rohan · فندق نُزُل الرياض | Nuzul Riyadh Hotel · 2026-07-24 · 2026-07-26"),
			("Outbound", "Template", "hotelpms_precheckin_link", "Sent",
			 "Rohan · https://demo.hotelpms.yemenfrappe.com/hotelpms/checkin/…"),
