...
Run it:
    bench --site hotelpms.localhost execute hotelpms.scripts.demo_aiosell.run
...
running the show:  ...execute hotelpms.scripts.demo_aiosell.reset
"""

import frappe
from frappe.utils import add_days, nowdate

from hotelpms.channels import provider_for
from hotelpms.channel_manager import _apply_event, ari_snapshot

DEMO_PROPERTY = "منتجع نُزُل الدرعية | Nuzul Diriyah Retreat"   # has a real Villa + a member STD
...
def preview(property=None, days=5):
	"""Show the OUT direction without live credentials: the exact JSON HotelPMS
	would POST to Aiosell's /update (inventory) and /update-rates (rates)."""
	import json
	from hotelpms.channels.aiosell import build_push_bodies
	...
	_line("═")
	print(f"  OUTBOUND PREVIEW · what HotelPMS would send Aiosell for {property}")
...
def push_to_sandbox(password, property=None, days=14):
	"""Push REAL HotelPMS availability + rates to Aiosell's shared SANDBOX and
	watch it land on live.aiosell.com.

	Points a HotelPMS connection at the sandbox hotel, maps HotelPMS's room types onto
	the sandbox's OWN room codes (executive, suite) so the numbers are visible,
	then fires HotelPMS's real push. Run with the sandbox password:
	    ...execute hotelpms.scripts.demo_aiosell.push_to_sandbox --kwargs '{"password":"THE_SANDBOX_PW"}'
	"""
	from hotelpms.channel_manager import push_ari, ari_snapshot
	from hotelpms.channels.aiosell import build_push_bodies
	frappe.set_user("Administrator")  # nosemgrep: frappe-setuser -- demo script runs as admin to seed and read demo data; not app runtime
	property = _pick_property(property)
	member_rt, villa_rt = _room_types(property)
	...
	# map HotelPMS room types -> the sandbox's own codes so they show on its board
	...
	_line("═")
	print(f"  HOTELPMS → AIOSELL SANDBOX   ({property}, next {days} days)")
	_line("═")
	print("HotelPMS room type  →  sandbox code:")
	for rt, rc, rp in sandbox_map:
		if rt:
			print(f"   {rt}  →  {rc} / {rp}")
	print(f"\nAbout to send {len(inv)} inventory + {len(rates)} rate blocks "
	      f"(HotelPMS's real availability & pricing).")
	try:
		res = push_ari(conn.name, days=int(days))
		print("RESULT:", res)
		print("\n✅ Sent. Refresh live.aiosell.com (Sandbox PMS · Rates & "
		      "Inventory) — EXECUTIVE/SUITE now show HOTELPMS's numbers.")
...
def run(property=None):
	...
	_line()
	print("STEP 1 · A guest books on Goibibo → Aiosell → HotelPMS")
	_webhook(conn, {
	...
	r = _res("DEMO-501")
	print(f"   → HotelPMS created reservation {r.name}: {r.guest_name} "
	      f"({r.channel}), {r.check_in_date}→{r.check_out_date}, ₹{r.amount_after_tax:.0f}")
	print("   Nobody typed it in. Open HotelPMS → Today/Reservations to show it.")
	...
	if villa_rt:
		_line()
		print("STEP 4 · Villa ↔ Room lockout (what HotelPMS pushes back to OTAs)")
