...
must get none of that: no فندق نُزُل الرياض | Nuzul Riyadh Hotel, no invented reservations, no
...
Run from the infra script, or by hand:
    ...
    bench --site ewa.hotelpms.yemenfrappe.com execute hotelpms.scripts.provision.setup \\
...
# Every role HotelPMS ships. A tenant gets the ones its modules need.
ROLES = ("Hotel Admin", "Front Desk", "Housekeeping", "Finance",
         "Revenue Manager", "HotelPMS Agent")
...
def configure_smtp(host: str, port: int = 587, login: str = "",
	...
	HotelPMS Cloud outbound mail is usually Resend SMTP
...
def status(property_name: str | None = None) -> dict:
	...
	mail = mail_ready()
	demo = frappe.db.exists("Property", "فندق نُزُل الرياض | Nuzul Riyadh Hotel")
	return {
		"properties": props,
