...
The shared sandbox at demo.hotelpms.yemenfrappe.com (and nightly) is for trying HotelPMS,
...
Guarded: refuses unless ``hotelpms_demo_mode`` is on *and* the site is a
...
PLAYGROUND_SITES = {
	"demo.hotelpms.yemenfrappe.com",
	"nightly.hotelpms.yemenfrappe.com",
	"hotelpms.yemenfrappe.com",
}
...
def is_playground() -> bool:
	if frappe.db.get_default("hotelpms_demo_mode") != "1":
		return False
...
def execute():
	...
	if not is_playground():
		frappe.throw(
			"Refusing to reset: this site is not a HotelPMS demo playground. "
			"Set hotelpms_demo_mode and use demo.hotelpms.yemenfrappe.com / nightly / localhost."
		)
	return reset()


def reset() -> dict:
	"""Wipe HotelPMS + leftover users, then reseed the sample hotel."""
	summary: dict = {"site": frappe.local.site, "wiped": {}}

	frappe.flags.ignore_permissions = True
	frappe.flags.in_import = True

	summary["wiped"]["hotelpms"] = _wipe_hotelpms_doctypes()
	...
	from hotelpms.scripts.seed_demo import execute as seed_demo
	seed_demo()

	# This deployment is the Arabic/English showcase. Preserve that polished
	# fixture after the stock reset instead of falling back to English-only
	# sample content. Other upstream playgrounds keep their normal seed.
	if frappe.db.get_default("hotelpms_demo_language") == "ar":
		from hotelpms.scripts.seed_arabic_demo import execute as seed_arabic_demo
		seed_arabic_demo()
	...
	msg = (
		...
		f"hotelpms={summary['wiped']['hotelpms']} core={summary['wiped']['core']} "
...
def _wipe_hotelpms_doctypes() -> int:
	rows = frappe.get_all(
		"DocType",
