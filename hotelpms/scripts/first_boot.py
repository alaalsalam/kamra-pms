...
"""WordPress-style first boot for a fresh HotelPMS site.

Called after ``bench new-site … --install-app hotelpms`` (install.sh, cloud-init,
...
def execute():
	"""Entry: ``bench --site <site> execute hotelpms.scripts.first_boot.execute``."""
	frappe.connect()
	_set_home_to_hotelpms()
	_set_admin_email()
	_set_host_name()
	_ensure_scheduler()
	frappe.clear_cache()
	print("hotelpms first_boot: ok")


def _set_home_to_hotelpms():
	"""Land visitors on /hotelpms, not empty Frappe Desk."""
	ws = frappe.get_doc("Website Settings")
	changed = False
	if not ws.favicon:
		ws.favicon = "/assets/hotelpms/hotelpms-mark.svg"
		changed = True
	# Frappe serves www/hotelpms.py at /hotelpms; home_page is the route name.
