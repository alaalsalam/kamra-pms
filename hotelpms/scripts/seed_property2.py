...
Run via bench console:
    from hotelpms.scripts.seed_property2 import execute; execute()
"""

import frappe

P2 = "أجنحة نُزُل العليا | Nuzul Olaya Suites"
P1 = "فندق نُزُل الرياض | Nuzul Riyadh Hotel"
...
def execute():
	if not frappe.db.exists("Property", P2):
		frappe.get_doc({
			...
			"email": "beach@hotelpms.local",
			"gstin": "29ABCDE1234F2Z4",
			"star_category": "Homestay",
			"showcase_description": "Six rooms, one cliff, endless sea. "
			                        "أجنحة نُزُل العليا | Nuzul Olaya Suites is barefoot luxury on the Gokarna coast.",
	...
	if not frappe.db.exists(
		"User Permission",
		{"user": "frontdesk@hotelpms.local", "allow": "Property"},
	):
		frappe.get_doc({
			"doctype": "User Permission",
			"user": "frontdesk@hotelpms.local",
			"allow": "Property",
			"for_value": P1,
			"apply_to_all_doctypes": 1,
		}).insert(ignore_permissions=True)
		print("frontdesk@hotelpms.local restricted to فندق نُزُل الرياض | Nuzul Riyadh Hotel")
