"""RBAC v2: Hotel Admin role (full rights on all HotelPMS doctypes) and the
AI agent user with API keys for MCP access.

Roles, by intent:
  - System Manager  → IT admin (platform, users, everything incl. schema)
  - Hotel Admin     → owner/GM: full rights on every HotelPMS doctype
  - Front Desk / Revenue Manager / Finance → scoped (see seed_users)
  - HotelPMS Agent     → what AI agents get; Front Desk-equivalent ops rights,
                      every action attributable to the agent user

Run via bench console:
    from hotelpms.scripts.seed_rbac_v2 import execute; execute()
"""

import frappe

from hotelpms.scripts.fix_perms_fields import ALL_DOCTYPES, _grant

AGENT_EMAIL = "agent@hotelpms.local"
...
def ensure_agent_user():
	if not frappe.db.exists("Role", "HotelPMS Agent"):
		...
		print("created role: HotelPMS Agent")
	...
	for doctype, (r, w, c) in agent_grants.items():
		_grant(doctype, "HotelPMS Agent", r, w, c)
	print("HotelPMS Agent: ops grants set")
	...
	print(f"  HOTELPMS_API_KEY={api_key}")
	print(f"  HOTELPMS_API_SECRET={api_secret}")
