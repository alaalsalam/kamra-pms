"""HotelPMS eval harness — deterministic checks over the governed tool layer.
...
def setup():
	...
	# doctype JSONs already carry the HotelPMS Agent role.
	if not frappe.db.exists("Role", "HotelPMS Agent"):
		frappe.get_doc({
			"doctype": "Role", "role_name": "HotelPMS Agent", "desk_access": 0,
		}).insert(ignore_permissions=True)
	if not frappe.db.exists("User", "agent@hotelpms.local"):
		frappe.get_doc({
			"doctype": "User", "email": "agent@hotelpms.local",
			"first_name": "HotelPMS", "last_name": "Agent", "enabled": 1,
			"user_type": "System User", "send_welcome_email": 0,
			"roles": [{"role": "HotelPMS Agent"}],
		}).insert(ignore_permissions=True)
	...
	for email, first, role in (
		("frontdesk@hotelpms.local", "Ravi", "Front Desk"),
		("hk@hotelpms.local", "Lakshmi", "Housekeeping"),
	):
...
def t10():
	from hotelpms.folio import post_room_night
...
def t11():
	from hotelpms.folio import split_folio, transfer_charge
...
def t14():
	from hotelpms import api
	from hotelpms.folio import post_room_night, split_folio
...
def t18():
	from hotelpms import api
	from hotelpms.folio import run_night_audit
...
def t19():
	from hotelpms import api
	from hotelpms.folio import close_folio, post_room_night
...
def t23():
	...
	from hotelpms.housekeeping import escalate_overdue_tasks
...
def t24():
	from hotelpms.crs import assert_property_access, permitted_properties
	u = "eval.pinned@hotelpms.local"
...
def t32():
	from hotelpms import api
	from hotelpms.pricing import demand_tier, forecast_occupancy, quote
...
def t40():
	from hotelpms import inventory, pos
...
def t41():
	...
	try:
		frappe.set_user("frontdesk@hotelpms.local")  # nosemgrep: frappe-setuser -- controlled user context switch; target user is validated and scope-limited in this flow
...
def t42():
	...
	from hotelpms import api, public_api as pub
...
def t43():
	from hotelpms import api, id_documents, public_api as pub
	...
	try:
		...
		assert frappe.db.get_value("Reservation", res.name,
		                           "precheckin_verified_by") == "frontdesk@hotelpms.local"
	...
	frappe.set_user("hk@hotelpms.local")  # nosemgrep: frappe-setuser -- controlled user context switch; target user is validated and scope-limited in this flow
	try:
...
def t34():
	import base64
	from hotelpms import api, public_api
...
def t45():
	from hotelpms.api import property_locale
	from hotelpms.public_api import _public_locale
...
def t46():
	...
	from hotelpms.localization import malaysia
	...
	from hotelpms.localization import uae
...
def t48():
	from hotelpms import whatsapp  # nosemgrep: frappe-monkey-patching-not-allowed -- offline eval harness stubs the outbound transport for tests; not a production code path
	...
	frappe.get_doc({
		...
		"meta_language": "en",
		"tpl_booking_confirmation": "hotelpms_booking_confirmation",
		"tpl_precheckin": "hotelpms_precheckin_link",
		"tpl_payment_request": "hotelpms_payment_request",
	}).insert(ignore_permissions=True)
	...
	try:
		...
		assert sent[0]["template"]["name"] == "hotelpms_booking_confirmation"
		params = sent[0]["template"]["components"][0]["parameters"]
		assert len(params) == 4 and params[1]["text"], params
		assert sent[1]["template"]["name"] == "hotelpms_precheckin_link"
		assert "/hotelpms/checkin/" in sent[1]["template"]["components"][0][
			"parameters"][1]["text"]
...
def t49():
	...
	points) and, worse, whitelists the helper. Sweep every hotelpms.* method
	the frontend references and prove each is a registered endpoint."""
	import pathlib
	import re

	src = pathlib.Path(frappe.get_app_path("hotelpms")).parent / "frontend" / "src"
	assert src.exists(), f"frontend source not found at {src}"
	calls = set()
	for f in src.rglob("*.ts*"):
		calls |= set(re.findall(r'"(hotelpms\.\w+\.\w+)"', f.read_text()))
	...
	leaked = [str(fn) for fn in frappe.whitelisted
	          if getattr(fn, "__module__", "").startswith("hotelpms.")
...
def t50():
	...
	from hotelpms.api import check_in, checkin_context
...
def t51():
	...
	from hotelpms.api import (_scrub_stay_ids, check_in, update_occupants,
	                       upload_occupant_id)
...
def t53():
	...
	from hotelpms import channel_manager as cm
	from hotelpms.channels import channex  # nosemgrep: frappe-monkey-patching-not-allowed -- offline eval harness stubs the outbound transport for tests; not a production code path
...
def _function(day_offset=60, **kw):
	from hotelpms import banquet as bq
...
def execute():
	...
	print(f"\n=== HotelPMS eval harness: {passed}/{len(RESULTS)} passed ===")
