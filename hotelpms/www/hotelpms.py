...
def get_context(context):
	...
	The front-end is built by Vite into hotelpms/public/frontend (served by Frappe
	at /assets/hotelpms/frontend/). We read that index.html at request time — so
	...
	"""
	index_path = frappe.get_app_path("hotelpms", "public", "frontend", "index.html")
	if not os.path.exists(index_path):
		frappe.throw(
			frappe._(
				"HotelPMS front-end is not built. Run "
				"<code>cd apps/hotelpms/frontend && yarn install && yarn build</code> "
				"(Frappe Cloud runs this automatically on deploy)."
			),
			title="HotelPMS not built",
		)
