...
Providers (hotelpms/channels/*) translate protocol; everything with
...
def _sellable(property: str, room_type: str, check_in, check_out) -> int:
	...
	from hotelpms.api import _block_hold
...
def _apply_villa_lockout(rows: list[dict], meta: dict) -> None:
	...
	Reservation.validate_villa_lockout; that one stops HotelPMS accepting a
...
@frappe.whitelist()
@require_roles("Revenue Manager", "Hotel Admin", "HotelPMS Agent")
...
def enqueue_property_push(property: str | None) -> None:
	...
	try:
		if not property or not _connections(property):
			return
		frappe.enqueue("hotelpms.channel_manager.push_all_ari", queue="long",
...
def webhook(connection: str, **kwargs):
	...
	/api/method/hotelpms.channel_manager.webhook?connection=CMC-00001
...
def _reservations_for_booking(property: str, booking_id: str) -> list[dict]:
	"""Every HotelPMS reservation for one OTA booking: the single-room case
...
def _apply_event(conn, e: dict) -> dict:
	...
	if e["event"] == "cancel":
		...
		from hotelpms.api import _do_cancel
...
def import_room_mappings(connection: str, dry_run: int = 0):
	...
	Auto-matches a provider room to a HotelPMS Room Type by code or name, then
...
@frappe.whitelist()
@require_roles("Revenue Manager", "Hotel Admin", "Front Desk", "HotelPMS Agent")
def connection_status(property: str):
	...
	for name in _connections(property, only_active=False):
		...
		out.append({
			...
			"webhook_url": f"/api/method/hotelpms.channel_manager.webhook"
			               f"?connection={c.name}",
		})
	return out
