"""HotelPMS Agent - the in-app assistant. BYOK, optional, governed.
...
from hotelpms.llm_compat import chat_payload, retry_chat_payload
import requests
...
SYSTEM = """You are HotelPMS Agent, the assistant for {property_name}, a hotel
running HotelPMS. Today is {today}. You help staff work faster: look
...
def _resolve(fn_name: str):
	"""A tool's target. Bare names live in hotelpms.api (where most of the
	...
	if "." in fn_name:
		...
		return getattr(importlib.import_module(f"hotelpms.{module}"), attr, None)
	from hotelpms import api
	return getattr(api, fn_name, None)


def _tool_allowed(name: str) -> bool:
	"""RBAC for HotelPMS Agent: a tool is only visible/callable when the
...
def _run_tool(name: str, args: dict, property: str):
	...
	# HotelPMS Agent tool calls are agent actions: accountability comes from
...
def ask(property: str, messages):
	"""One HotelPMS Agent turn: history in, answer out. The model may call
...
def ask_stream(property: str, messages):
	"""Streaming HotelPMS Agent turn (Server-Sent Events). Governed tools run FIRST
...
HELP_SYSTEM = """You are the HotelPMS help assistant. You explain HOW to use
HotelPMS - an open-source, AI-native hotel PMS - to hotel staff. You do NOT act on
hotel data (HotelPMS Agent does that); you give short, concrete how-to
answers and point to where things live in the app.

What HotelPMS does and where to find it:
...
- Housekeeping: room board + a phone app at /hotelpms/hk.
...
- Roles: a System Admin (IT - users, API keys, Frappe Desk) versus a Hotel
  ...
  Admin → Manage Users.
- HotelPMS Agent (the sparkle button) can act - quote, book, check in,
...
the docs (github.com/YemenFrappe/hotelpms/tree/main/docs)."""
...
def help_ask(property: str, messages):
	...
	how to use HotelPMS, grounded in the app's features. Reuses the property's
