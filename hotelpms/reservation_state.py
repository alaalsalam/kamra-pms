...
# hotelpms.siu.availability.LIVE_STATUSES).
...
def assert_transition(old_status: str | None, new_status: str | None):
	...
	Bypassed when ``frappe.flags.hotelpms_status_transition`` is set (APIs that
	own the transition) or during cancel via ``hotelpms_cancelling``.
	"""
	import frappe
	from frappe import _

	if frappe.flags.hotelpms_status_transition or frappe.flags.hotelpms_cancelling:
		return
	old = old_status or ""
	new = new_status or ""
