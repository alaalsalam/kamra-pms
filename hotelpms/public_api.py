...
from hotelpms.booking_slugs import resolve_public_slug, slugify
...
def site_info():
	...
	demo login accounts.
	"""
	return {"demo_mode": frappe.db.get_default("hotelpms_demo_mode") == "1"}
...
def default_property():
	...
	Each HotelPMS deploy is single-tenant: one site = one hotel/villa. The
...
def search_stay(property: str, check_in_date: str, check_out_date: str,
	...
	# MCP / HotelPMS Agent tool; guests need the same availability math without
	# role gate, so this calls the same underlying helpers directly.
	from hotelpms.api import _available_rooms_raw, _block_hold
	from hotelpms.pricing import quote
	from hotelpms.siu.availability import has_active_sius, sellable_count
...
GUEST_AGENT = "agent@hotelpms.local"
...
def book(property: str, room_type: str, check_in_date: str,
	...
	from hotelpms.api import create_booking
	from hotelpms.pricing import quote as price_quote
	from hotelpms.reservation_state import resolve_instant_status
	...
	frappe.set_user("agent@hotelpms.local")  # governed writer for guest bookings  # nosemgrep: frappe-setuser -- controlled user context switch; target user is validated and scope-limited in this flow
	try:
...
def access_info(token: str):
	...
	from hotelpms.access import guest_access_info
...
def qr_order(outlet: str, items, room: str | None = None,
	...
	frappe.set_user("agent@hotelpms.local")  # governed writer, like public bookings  # nosemgrep: frappe-setuser -- controlled user context switch; target user is validated and scope-limited in this flow
	try:
...
def hosting_enquiry(full_name: str, email: str, phone: str = "",
	...
	try:
		frappe.sendmail(
			recipients=["hello@hotelpms.yemenfrappe.com"],
			subject=f"HotelPMS {doc.interest or 'Cloud'} enquiry: {doc.full_name}"
