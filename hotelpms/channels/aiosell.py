...
`/api/v2/cm` wire format on both sides and hands HotelPMS's normalized shapes to
hotelpms.channel_manager, where every consequence (booking creation, availability,
...
def push_ari(conn, snapshot) -> tuple[bool, str]:
	...
	room types. `snapshot` is hotelpms.channel_manager.ari_snapshot's shape, with
...
def fetch_property_details(conn) -> dict:
	...
	map HotelPMS's room types onto. Call this first, once per property."""
	import requests
	from requests.auth import HTTPBasicAuth
...
# ---------------------------------------------------------------------------
# Webhook: AioSell -> HotelPMS
# ---------------------------------------------------------------------------

def parse_webhook(conn, payload) -> list[dict]:
	"""AioSell reservation webhook -> HotelPMS's normalized events.
...
WEBHOOK_PATH = "/api/method/hotelpms.channels.aiosell.reservation_webhook"
...
def reservation_webhook(**kwargs):
	...
	/api/method/hotelpms.channels.aiosell.reservation_webhook
	...
	frappe.enqueue("hotelpms.channel_manager.process_webhook_events",
