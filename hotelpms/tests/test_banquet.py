...
from hotelpms.tests.fixtures import PROPERTY, build, enquiry
...
class TestModuleGate(BanquetTestCase):
	"""A property runs only the parts of HotelPMS it bought - but two of them
	can't be given up, or the site becomes unadministrable."""

	def test_a_property_runs_only_what_it_switched_on(self):
		from hotelpms.api import enabled_modules, set_enabled_modules
	...
	def test_an_empty_setting_means_everything(self):
		from hotelpms.api import ALL_MODULES, enabled_modules
	...
	def test_a_module_that_does_not_exist_is_refused(self):
		from hotelpms.api import set_enabled_modules
