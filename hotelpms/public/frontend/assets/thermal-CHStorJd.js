import{w as o,t as p,o as r}from"./index-B0FfYkEE.js";const u=`
...
`,s=t=>String(t??"").replace(/[&<>"]/g,n=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"})[n]),l=t=>Number(t??0).toLocaleString(o(),{minimumFractionDigits:2,maximumFractionDigits:2});function $(t,n){document.querySelectorAll("iframe[data-hotelpms-print]").forEach(d=>d.remove());const a=document.createElement("iframe");a.setAttribute("data-hotelpms-print","1"),a.setAttribute("title",t),a.style.cssText="position:fixed;right:0;bottom:0;width:0;height:0;border:0;opacity:0",document.body.appendChild(a);const e=a.contentWindow;if(!e)return a.remove(),!1;e.document.open(),e.document.write(`<!doctype html><html><head><title>${s(t)}</title><style>${u}</style></head><body>${n}</body></html>`),e.document.close();const i=()=>{try{a.remove()}catch{}};return e.onafterprint=i,setTimeout(()=>{try{e.focus(),e.print()}catch{i()}},200),setTimeout(i,12e4),!0}function g(t){const n=new Date().toLocaleString(o(),{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"}),a=t.items.map(e=>`<tr><td class="num b" style="width:24px">${Math.round(e.qty)}×</td><td class="b">${s(e.item_name)}</td></tr>`+(e.instructions?`<tr><td></td><td class="ins">» ${s(e.instructions)}</td></tr>`:"")).join("");return`
    ...
    <div class="cut">- cut -</div>`}function h(t){const n=new Date().toLocaleString(o(),{day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"}),a=t.items.map(e=>`<tr><td class="num b" style="width:26px">${Math.round(e.qty)}×</td><td class="b">${s(e.item_name)}<div class="sm" style="font-weight:400">${s(e.service_type)}</div></td></tr>`).join("");return`
    ...
    ${t.discount_amount?i("Discount",-t.discount_amount):""}
    ${p()==="VAT"?i(`VAT @ ${t.gst_rate}%`,t.cgst+t.sgst,"sm"):i(`CGST @ ${t.gst_rate/2}%`,t.cgst,"sm")+i(`SGST @ ${t.gst_rate/2}%`,t.sgst,"sm")}
    <div class="rule"></div>
    ${i("TOTAL",t.grand_total,"b lg")}
    ...
    <div class="cut">- cut -</div>`}export{y as b,g as k,h as l,$ as p};
