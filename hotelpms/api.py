"""Whitelisted API for the HotelPMS front-desk UI.
...
from hotelpms.authz import require_it_admin, require_roles
...
@frappe.whitelist()
@require_roles("Front Desk", "Housekeeping", "Revenue Manager", "Finance",
	"Restaurant POS", "Kitchen", "HotelPMS Agent")
def enabled_modules(property: str) -> list:
	"""Which parts of HotelPMS this property runs. Empty setting = all of
...
@frappe.whitelist()
@require_roles("Front Desk", "HotelPMS Agent", "Hotel Admin", "System Manager")
...
@frappe.whitelist()
@require_roles("Revenue Manager", "HotelPMS Agent")
def set_room_rate(property: str, room_type: str, start_date: str,
	...
	from hotelpms.pricing import demand_tier
...
def setup_property(payload):
	...
	p = payload["property"]
	from hotelpms.property_presets import apply_kind_defaults, skip_meal_plans
	...
	prop.insert()
	try:
		from hotelpms.turnover import ensure_default_profile
		from hotelpms.property_presets import KIND_STR, normalize_kind
	...
	siu_stats = {}
	try:
		from hotelpms.siu.units import backfill_property
...
def record_advance(reservation: str, amount: float, mode: str = "UPI",
	...
	from hotelpms.folio import _recalculate, open_folio

	# Promote Pending Payment / Held → Confirmed before opening folio.
	if res.status in ("Pending Payment", "Held"):
		frappe.flags.hotelpms_status_transition = True
		try:
			res.status = "Confirmed"
			res.hold_expires_on = None
			res.save(ignore_permissions=True)
		finally:
			frappe.flags.hotelpms_status_transition = False
		res.reload()
...
def withhold_security_deposit(reservation: str, amount: float, reason: str,
	...
	from hotelpms.deposit import withhold_deposit
...
def refund_security_deposit(reservation: str, amount: float | None = None,
                            reference: str | None = None):
	from hotelpms.deposit import refund_deposit
...
def release_access_instructions(reservation: str, force: int = 0):
	from hotelpms.access import release_access
...
def confirm_pending_reservation(reservation: str):
	...
	try:
		...
		frappe.flags.hotelpms_status_transition = False
	try:
		from hotelpms.deposit import ensure_deposit_for_reservation
...
def folio_payment_link(folio: str):
	from hotelpms.payments import create_payment_link
...
@frappe.whitelist(methods=["POST"])
@require_roles("Front Desk", "Housekeeping", "HotelPMS Agent")
...
def get_folio(reservation: str):
	...
	if not name:
		...
		if res.status not in ("Checked In", "Checked Out"):
			return None
		from hotelpms.folio import open_folio
		name = open_folio(res)
...
def _resolve_charge_gst(property: str, charge_type: str, description: str,
	...
	if any(w in text for w in _FNB_WORDS):
		from hotelpms.folio import _fnb_gst
...
def post_stay_charge(reservation: str, charge_type: str, description: str,
	...
	from hotelpms.folio import target_folio
	is_alcohol = 1 if int(is_alcohol or 0) else 0
...
@frappe.whitelist()
@require_roles("Finance", "Revenue Manager", "Front Desk", "HotelPMS Agent")
...
def split_folio(reservation: str, folio_type: str = "Extra"):
	from hotelpms.folio import split_folio as _split
...
def transfer_folio_charge(from_folio: str, charge_row: str, to_folio: str):
	from hotelpms.folio import transfer_charge
...
def transfer_folio_charges(from_folio: str, charge_rows, to_folio: str):
	...
	from hotelpms.folio import transfer_charges
...
def split_folio_charge(from_folio: str, charge_row: str, to_folio: str,
	...
	from hotelpms.folio import split_charge
...
def group_master_folio(group_booking: str):
	...
	from hotelpms.folio import open_group_folio
...
def post_allowance(folio: str, amount: float, reason: str,
	...
	_pin_guard(folio, pin)
	from hotelpms.folio import post_allowance as _allow
...
def part_settle_folio(folio: str, pin: str | None = None):
	...
	_pin_guard(folio, pin)
	from hotelpms.folio import part_settle as _settle
...
def cancel_invoice(folio: str, reason: str, pin: str | None = None):
	...
	_pin_guard(folio, pin)
	from hotelpms.folio import cancel_invoice as _cancel
...
def folio_invoice(folio: str):
	...
	"""
	from hotelpms import localization as loc
...
def run_night_audit(property: str, business_date: str | None = None):
	...
	from hotelpms.folio import run_night_audit as _run
...
@frappe.whitelist()
@require_roles("Front Desk", "HotelPMS Agent", "Finance", "Revenue Manager")
...
@frappe.whitelist()
@require_roles("Front Desk", "Finance", "Revenue Manager", "HotelPMS Agent")
...
def upload_id_document(reservation: str, data: str):
	...
	from hotelpms.id_documents import store_id_document
...
def checkin_context(reservation: str):
	...
	if not res.room and res.status == "Confirmed":
		...
		from hotelpms.allocation import suggest_allocation
...
def upload_occupant_id(reservation: str, row: str, image: str):
	...
	from hotelpms.id_documents import _decode, _sanitise
...
def upload_guest_document(guest: str, kind: str, image: str):
	...
	from hotelpms.public_api import _save_id_image
...
def _scrub_stay_ids(res):
	...
	from hotelpms.id_documents import discard_id_document
...
def _cancellation_terms(res):
	...
	from hotelpms.folio import policy_fee
...
def _do_cancel(res, reason: str = "Guest request", note: str | None = None,
	...
	if terms["inside_window"] and not int(waive_fee or 0) \
		...
		from hotelpms.folio import post_policy_fee
		fee = post_policy_fee(
			res, terms["fee_basis"],
	...
	frappe.flags.hotelpms_cancelling = True
	try:
...
def availability_calendar(property: str, start_date: str | None = None, days: int = 14):
	...
	from hotelpms.pricing import occupancy_rate, season_adjust
	from hotelpms.siu.availability import capacity_by_night, has_active_sius
...
def tape_chart(property: str, start_date: str | None = None, days: int = 14):
	...
	from hotelpms.pricing import demand_tier, forecast_occupancy
...
def send_precheckin_link(reservation: str, channel: str = "WhatsApp"):
	...
	link = f"{frappe.utils.get_url()}/hotelpms/checkin/{res.precheckin_token}"
	phone = frappe.db.get_value("Guest", res.guest, "phone") if res.guest else None
	sent = False
	if phone:
		try:
			from hotelpms.agents_channels import send_outbound
...
@frappe.whitelist()
@require_roles("Front Desk", "Revenue Manager", "HotelPMS Agent")
...
def get_quote(property: str, room_type: str, check_in_date: str,
	...
	from hotelpms.pricing import quote
...
def create_booking(property: str, room_type: str, check_in_date: str,
	...
	from hotelpms.crs import assert_property_access
	from hotelpms.reservation_state import find_by_idempotency, lock_sius_for_booking
	...
	voucher = None
	if voucher_code:
		from hotelpms.pricing import validate_voucher
		from frappe.utils import date_diff
...
@frappe.whitelist()
@require_roles("Front Desk", "HotelPMS Agent", "Revenue Manager")
...
@frappe.whitelist()
@require_roles("Front Desk", "HotelPMS Agent", "Revenue Manager", "Hotel Admin")
def inventory_availability(property: str, check_in_date: str, check_out_date: str,
	...
	from hotelpms.siu.availability import availability as avail
	return avail(
		property,
		siu=siu or None,
...
def available_rooms(property: str, room_type: str, check_in_date: str,
	...
	rooms = None
	from hotelpms.siu.availability import has_active_sius
	if has_active_sius(property, room_type):
		from hotelpms.siu.availability import available_rooms_via_siu
