...
needs to do is:
    ...
    1. POST to /api/method/hotelpms.agents_channels.voice_webhook or
       messaging_webhook with an HMAC-SHA256 signature of the raw body
       in the X-HotelPMS-Signature header (hex-encoded).
    2. Include phone_number of the property line in the payload so
       HotelPMS can route to the right Channel Provider Connection.
...
def messaging_webhook(payload: str | dict | None = None) -> dict:
	...
	directly or POSTs to a follow-up endpoint to receive HotelPMS's reply.
	"""
	body = _parse_payload(payload)
...
def send_outbound(
	...
	*,
	...
	# native API. HotelPMS doesn't need to know the difference between Meta
...
def _verify_signature(secret: str, body: dict) -> bool:
	sig_header = (
		frappe.get_request_header("X-HotelPMS-Signature")
