"""The HotelPMS Marketplace: what's in your plan, and what you can plug in.

Three kinds of card, three install models - never conflated:

  module     a HotelPMS app that ships in this codebase, open and included
             (no install, no lock).
...
def registry(property: str):
	...
	return [
		...
		},
		{
			...
			"cards": [
				_module("WhatsApp - Meta Cloud API",
				        ...
				        "per conversation; HotelPMS adds nothing.",
		...
		},
		{
			...
			"blurb": "HotelPMS is agent-ready. HeyKoala is the AI that answers your "
			         "phone and WhatsApp - it books, quotes and handles requests "
			         "24x7 using HotelPMS's governed tools, and every action is "
		...
		{
			"category": "Accounting",
			"blurb": "Push closed folios to your books - HotelPMS keeps the "
...
def connect_heykoala(property: str, channel: str, phone_number: str):
	...
	return {
		...
		"webhook_url": f"{base}/api/method/hotelpms.agents_channels.{method}",
		"webhook_secret": secret,
		"signature_header": "X-HotelPMS-Signature",
		"signature_note": "HMAC-SHA256 of the JSON body (keys sorted), hex, "
		                  "sent in the X-HotelPMS-Signature header.",
	}
