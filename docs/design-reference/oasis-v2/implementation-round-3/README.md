...
## Endgame verification (2026-09-07) — closing the checkpoint gaps
Four targeted checks before consolidating:
1. **EN-mode gate (AR+EN parity).** Switched to English on `/reports` (header "Manager flash") and `/settings` (header "Property"): **no stranded Arabic UI labels** — the only Arabic is bilingual property names (data) + the "العربية" language-name in the picker. The `t()`/`fill()` usage restores to English correctly (no non-subscribing-node stranding). Note: money renders as `ر.س` + Arabic-Indic digits even in EN mode — a pre-existing app-wide `moneyLocale()` choice in `lib/money.ts` (not a round-3 regression); flag if the client wants Western digits/`SAR` under EN.
2. **Probed the never-opened Pkg 7/8 screens** (Hotel Admin): **Kitchen** (منطقة تسليم المطبخ), **Inventory** (مخزون المطبخ), **Banquet** (الحفلات والمآدب), **Channel Manager** (مدير القنوات), **Agents** (مساعد HotelPMS) — all healthy (RTL, no overflow, 0 app errors, no in-page ₹). **Activity** (سجل النشاط) renders fine but shows the **backend ₹** in log entries (same systemic finding). **Outlets** (منافذ البيع) renders the **permission-denied** state for Hotel Admin — correct frontend behavior; whether Hotel Admin *should* hold POS-Outlet read is a backend perm-policy question (same class as the Lost&Found finding). Minor: Kitchen shows an English "N tickets" count badge (count-label i18n).
3. **`/stay/:slug` happy path — WORKS.** The earlier "Listing not found" was legitimate (`resolve_public_slug` throws `DoesNotExistError`/404 when the listing's property has `booking_engine_enabled=0`). With a booking-enabled listing slug (`غرفة-نزل-ديلوكس-nuzul-deluxe-room`) the page renders fully (title, room cards, RTL, no overflow, no ₹). `/checkin/:token` remains unverifiable read-only (the token field is query-blocked; needs a real pre-arrival token).
4. **Laundry data-repair — none needed.** Queried all Laundry Rate rows: **zero** have an Arabic `service_type`, so the `value={s}` fix prevents future corruption and no existing rows need repair. Finding is complete.

**Amended "remaining":** every route was opened and is either done, verified-healthy, correct-by-design (denied states), or backend-lane. Frontend long-tail is now just scattered count/label i18n (e.g. Kitchen "N tickets", Assistant block) — non-blocking. This is a defensible completion point for the frontend redesign; the high-impact remaining work (systemic ₹→SAR, guest_journey/activity i18n, Lost&Found/Outlets perms, aggregate KPI endpoints) is the backend phase.
