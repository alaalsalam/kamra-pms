...
| `/hk` | HkApp | **Tested** (live, mobile) | (this commit) | mobile HK phone app. Added the missing **error/permission + loading states**: `load()` had no `.catch`/skeleton, so a permission failure showed a silent blank board — now a signed-in user without HK/Front-Desk/Agent (verified live as Finance) gets a "Couldn't load your board" + retry card, and a loading skeleton shows while fetching. Empty states translated (the "mine" compound uses `fill()` + an inline "available rooms" button); 44px header touch targets + 48px bottom-nav; logical props (ms-auto, end-1.5). Verified live 390 AR (self-auth as Housekeeping → translated empty; Finance → error card), no overflow, RTL correct. Backend untouched |
| `/laundry` | Laundry | **Tested** (live) | (this commit) | Not a rewrite (queue-first board already clean/translated). Added a **board-level onboarding** when the whole board is empty (explains pickup→processing→delivery, role-gated "Set laundry prices" → Price-menu tab) in place of three separate empty columns. Leak-scan fixed the Price-menu + Billing tabs (rate-card subtitle, "No rates yet — add the first item.", "Nothing delivered yet.", "Billed orders", "Recent laundry billing", "Last 30 days" via a single text node). Verified live gm 1024/390 AR: board onboarding + Price-menu + Billing tabs translated, no overflow. Backend untouched |
| `/lost-found` | ResourceScreen(lostFoundConfig) | **Tested** (live denied + populated/onboarding via mock) | (this commit) | Fixed the config English leaks (`Kind`/`Logged` column + `Kind` filter labels + condition/status values → AR keys). Added onboarding (no prerequisite master data — guides to New). **Live**: gm (Hotel Admin) is denied read on `Lost And Found Item` at the backend (the reference's raw "عدم كفاية الإذن Lost And Found Item" banner) → now the shared ResourceScreen renders a graceful translated **permission-denied** panel + hides New. Verified: 403-mock → denied panel; populated route-mock → 3 rows with translated Kind/Status badges; onboarding wired (reachable when a role has live read + empty data). ⚠️ **Backend note (out of frontend scope):** the repo doctype JSON grants Hotel Admin read but the live backend denies gm — a deployed-permission / property-scope discrepancy for the owner to check |
...
| Route | Component | Status | Commit | Notes |
|---|---|---|---|---|
| `/stay/:slug` | PublicListing | **Tested** (live as Guest, incl. one real booking then deleted) | (this commit) | **Guest browse→book fix** (reports B+C, plan 0006). (B) Backend permission fix: public `book()` quoted config doctypes as Guest → `PermissionError` when a meal plan was included; now quotes under the governed booking agent (`820b0d5`) + regression test. (C) Added an explicit availability-result step (available/sold-out/error-retry/checking — sold-out was silently blank before), moved a full **services** picker into the sheet (meal-plan selector, add-ons/experiences steppers, special requests, estimated total), and **stopped auto-selecting a paid meal plan** (guest was silently charged; est. ₹5,650→₹5,250 room-only). Gated the estimate on a real quote; bdi-wrapped the confirmation Total; reset booking fields on close. Full AR i18n incl. pre-existing guest-page leaks (Back to property, Where you'll be, Host & caretaker, Coordinated by, Up to N guests). **Live-verified** on `hotelpms.yemenfrappe.com` (property نون هوتيل / listing `standard`) at 390 + 1024, AR + EN; one real HTTP booking (Breakfast + special request) → 200, correct pricing, stored, then reservation + deposit + guest **deleted, counts verified back to baseline**. **Data (not code) note:** property shows ₹ instead of SAR ﷼ — currency unset in Property config, owner setting. **Follow-ups (plans 0007–0009):** date-robustness pass — sanitize inverted/past/garbage URLs (self-healing), and a radical date-editing fix so the guest can freely type day/month/year (the fetch boundary skips non-bookable ranges + shows a "check your dates" hint, and never mutates the inputs). Verified live with real keyboard typing at 390 AR. Same guards mirrored to `/book` (PublicBooking). |
